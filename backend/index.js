...
async function downloadTikTok(tiktokUrl) {
  ...
  if (!html) throw new Error('Gagal mendapatkan data.');
  ...
  if (mp4Links.length > 0) {
    return { type: 'video', title, mp4Links, mp3Link };
  }

  if (imageLinks.length > 0) {
    return { type: 'image', title, images: imageLinks, mp3Link };
  }

  throw new Error('Tidak ada data valid, mungkin link salah.');
}
